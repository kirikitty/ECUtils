...
@interface RFBlockActionSheet () <UIActionSheetDelegate>
...
@implementation RFBlockActionSheet
...
- (void)setBlock:(RFActionSheetBlock)block atButtonIndex:(NSInteger)buttonIndex
{
    ...
    RFActionSheetBlock p = [block copy];
...
{
    RFActionSheetBlock block = [self blockAtButtonIndex:buttonIndex];
    if (block) {
        block = [block copy];
...
#pragma mark - Utils
- (NSInteger)addButtonWithTitle:(NSString *)title action:(RFActionSheetBlock)action
{
